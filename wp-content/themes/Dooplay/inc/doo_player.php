...
class DooPlayer{
     ...
     * @version 1.0
     */
    public static function viewer($post, $type, $players, $trailer, $size, $views, $ads = false, $image = false, $movie_title = "", $tv = false, $season = 0, $episode = 0, $videospider_first = true){
        if($size == 'regular'){
        ...
        if(1==1){
			...
			if($tv) {
				...
				$videospider_url = get_template_directory_uri().'/vs_player.php?id='.$post.'&tv=1&s='.$season.'&e='.$episode;
				
			} else {
				
				$videospider_url = get_template_directory_uri().'/vs_player.php?id='.$post.'&tv=0&s=0&e=0';
				
			}	
			
			if($videospider_first) {
				
				$html .='<li id="player-option-1" class="dooplay_player_option vs" data-post="vs" data-type='.$type.' data-nume="vs" data-vs="'.$videospider_url.'">
							<i class="icon-play3"></i>
							<span class="title">'.$movie_title.'</span>
							<span class="server">openload.co</span>
				...
				$html .='<li id="player-option-'.$num.'" class="dooplay_player_option vs" data-post="vs" data-type='.$type.' data-nume="vs" data-vs="'.$videospider_url.'">
							<i class="icon-play3"></i>
							<span class="title">'.$movie_title.'</span>
							<span class="server">openload.co</span>
							<span class="flag"><img src="'.DOO_URI.'/assets/img/flags/en.png"></span>
